...
def safe_eval(expr, variables):
    safe_dict = {
        ...
        'sum': sum, 'pow': pow, 'Decimal': Decimal
    }
...
def execute_single_formula(formula, row):
    try:
        ...
        logging.error(f"Error in formula execution for '{formula.expression}': {str(e)}")
        raise HTTPException(status_code=400, detail=f"Error in formula execution for '{formula.expression}': {str(e)}")
